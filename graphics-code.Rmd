...
track_short_iaaf <- all_iaaf %>%
  filter(grepl("100m|200m|400m|110m|4x100m", Event) & nchar(Result) < 6) %>%
...
track_medium_iaaf <- all_iaaf %>%
  filter(grepl("800m|1500m|5000m|10000m|3000mh|4x400m", Event)) %>%
  mutate(Time = Result) %>%
...
short_track_records <- iaaf_wr %>%
  filter(grepl("100m|200m|400m|110m", Event) & nchar(Result) < 6) %>%
